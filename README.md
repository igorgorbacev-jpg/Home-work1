...
│
├── main.py
├── dirty_main.py          # (необязательное задание, если надо)
├── requirements.txt
│
└── application/
    ├── __init__.py
...
Примечание: файлы __init__.py делают папки Python-пакетами. Даже если они пустые — они обязательны для корректного импорта в более старых версиях Python и улучшают совместимость.
